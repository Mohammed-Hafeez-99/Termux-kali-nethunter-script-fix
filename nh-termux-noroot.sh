...
# SHA256SUMS file will be downloaded
}
function prepare_fs() {
    unset KEEP_CHROOT
    if [ -d ${CHROOT} ]; then
...
function get_url() {
    ...
    SHA_URL="https://kali.download/nethunter-images/current/rootfs/SHA256SUMS"
}
...
"
        ...
        # Extract expected checksum from SHA256SUMS file
        # Use pattern matching since IMAGE_NAME might not have version number
        expected_checksum=$(grep "${wimg}-${SYS_ARCH}.tar.xz" SHA256SUMS | awk '{print $1}')

        if [ -z "$expected_checksum" ]; then
            printf "${red}[!] Checksum not found for ${wimg}-${SYS_ARCH} in SHA256SUMS file
${reset}"
            exit 1
        fi

        # Get the full filename from SHA256SUMS
        full_filename=$(grep "${wimg}-${SYS_ARCH}.tar.xz" SHA256SUMS | awk '{print $2}')

        printf "${blue}[*] Expected file: ${full_filename}${reset}
"
        printf "${blue}[*] Calculating SHA256 checksum for ${IMAGE_NAME}...${reset}
"
        ...
        if [ "$expected_checksum" == "$actual_checksum" ]; then
            printf "${green}[+] Checksum verified successfully!${reset}
"
        else
            printf "${red}[!] Checksum mismatch!
"
            printf "[!] Expected: $expected_checksum
"
            printf "[!] Got:      $actual_checksum
"
            printf "[!] Rootfs corrupted. Please run this installer again or download the file manually
${reset}"
            exit 1
        fi
    fi
